...
// 指定したドライブの空き領域にバイナリ・ゼロのファイルを作成した後に削除
function MakeZeroFillFile(DriveLetter, FileSize, bitStr) {
// JScriptの変数に格納できる最大文字数は 16MiB - 1Byte
// "ADODB.Stream"で扱える１ファイルの最大サイズは 2GiB - 1bit
// だったため、文字数を8MiBにファイルサイズを1GiBに制限します。
...
# 　　されるように変更しています。
# 　・"CMD"をJScriptタイプのバッチファイルと決めておくと区別しやすくなります。
...
# 　※"PATH"の設定と同じようにドライブ名は書き換えられます。
#
# ◆"RelativeMKLink(リンクファイル, リンク元ファイル)"の行があった場合
# 　リンクファイル名でシンボリックリンクを作成します。
# 　ファイルは絶対パスで指定してください。
# 　リンクファイルはリンク元ファイルとの相対PATHで作成されるため、ドライブが変わった場合でも
# 　正常に機能します。
# 　※"PATH"の設定と同じようにドライブ名は書き換えられます。
